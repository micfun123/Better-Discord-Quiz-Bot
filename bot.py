...
async def send_question(ctx, quiz_instance):
    if quiz_instance.current_question_index >= len(quiz_data[quiz_instance.quiz_name]["questions"]) - 1:
    ...
    if total_votes != 0:
        ...
        for option in quiz_instance.votes:
            if isinstance(quiz_instance.votes[option], int):
                await ctx.send(f"{option} | {quiz_instance.votes[option]} | {quiz_instance.votes[option] / total_votes * 100:.2f}%")
